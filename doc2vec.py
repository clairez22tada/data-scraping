...
def load_glove(filename):
    ...
    with open(filename) as f:
        while True:
            ...
            if l[0] not in ENGLISH_STOP_WORDS:
                output[l[0]] =[float(i) for i in l[1:]]
        return output
...
def split_title(text):
    ...
    """
    return tuple(t for t in text.split("\n",1))
...
def load_articles(articles_dirname, gloves):
    ...
    for f in txt_files:
        s = get_text(f)
        output.append([f, split_title(s)[0], split_title(s)[1], doc2vec(split_title(s)[1], gloves)])
    return output  
...
def doc2vec(text, gloves):
    ...
    for word in word_list:
        if gloves.get(word, None) is  None:
            continue
        N+=1
        if centroid is None:
            centroid = gloves[word]
        else:
            centroid = [x+y for x,y in zip(centroid, gloves[word])]
    return tuple(x/N for x in centroid)
...
def distances(article, articles):
    ...
    output = []
    for a in articles:
        distance = np.sqrt(sum([(x-y)**2 for x, y in zip(article[3], a[3])]))
        output.append((distance, a))
    return output
...
def recommended(article, articles, n):
    ...
    """
    ## YOUR CODE HERE
    sorted_articles =  sorted([l for l in distances(article, articles) if l[1][0]!=article[0]])[:n]
    return [[a[1][0].split("/")[-2], a[1][0].split("/")[-1],a[1][1]] for a in sorted_articles]
...
def main():
    ...
    with open('articles.pkl', 'wb') as file:         
        pickle.dump([[a[0].split("/")[-2], a[0].split("/")[-1], a[1], a[2]] for a in articles], file) 
    ...
    recs = {}
    for a in articles:
        recs[(a[0].split("/")[-2], a[0].split("/")[-1])] = recommended(a, articles, 5)
